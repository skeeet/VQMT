...
void VideoYUV::getLuma(cv::cuda::GpuMat &local_luma, int type) {

    cv::Mat cpumat(height, width, CV_8UC1, this->luma);

    gpuBuff.upload(cpumat);
    gpuBuff.convertTo(local_luma, CV_32F);
}
